...
export default function App() {
  ...
  const processTimeline = (data) => {
    ...
    const startRange = new Date(now); startRange.setDate(now.getDate() - now.getDay() + (now.getDay() === 0 ? -6 : 1));
...
<RNStatusBar barStyle="light-content" backgroundColor="#0f172a" />
  ...
  {activeTab === 'plan' ? (
    ...
    const isToday = item.data === todayDateStr; // Check if it's today

    const cleanDescription = item.opis ? item.opis.replace(/<[^>]*>?/gm, '').trim() : '';

    return (
        <View style={[
          styles.card,
      isRace && styles.raceCard,
    isToday && !isRace && styles.todayCard // Apply today style (unless it's a race, let race style win)
  ]}>
  <View style={[
          styles.accent,
    ...
    {isToday ? `TODAY • ${item.data}` : item.data}
  </Text>
    <Text style={[styles.cardTitle, isRace && styles.raceTitle]}>
    {isRace ? `${item.nazwa_start}` : item.nazwa_trening}
  </Text>

    {cleanDescription ? (
        <Text style={styles.cardDetails}>{cleanDescription}</Text>
    ) : null}
  </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  // -- NEW TODAY HIGHLIGHT STYLES --
  todayCard: { borderColor: '#10b981', borderWidth: 1, backgroundColor: '#064e3b' }, // Dark emerald background with vibrant border
  todayAccent: { backgroundColor: '#10b981' }, // Emerald accent line
  todayDate: { color: '#10b981', fontWeight: '900' }, // Emerald date text
  ...
  cardDate: { color: '#64748b', fontSize: 11, fontWeight: '800', marginBottom: 4 },
