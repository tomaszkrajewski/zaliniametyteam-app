...
export default function TrainingModal({visible, date, events, onClose, onRefresh}) {
    ...
    const allGarminEvents = events
        ...
        .map(e => {
            ...
            let parsedLaps = [];
            try {
                parsedSummary = e.summary ? JSON.parse(e.summary) : {};
            } catch (err) {
            }
            try {
                parsedLaps = e.laps ? JSON.parse(e.laps) : [];
            } catch (err) {
            }
    ...
    useEffect(() => {
        ...
        const hideSub = Keyboard.addListener('keyboardDidHide', () => {
            // Nie robimy tutaj ScrollTo, zapobiega to "skakaniu" przy uciekającej klawiaturze
        });
    ...
    return (
        <Modal visible={visible} animationType="slide" transparent={true} onRequestClose={onClose}>
            {/* NAPRAWA: Zmienione behavior w Modalu dla Androida ("height") */}
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.modalOverlay}>
                <View style={styles.modalContent}>
                    ...
                    >
                            ...
                            return (
                                <View key={`run-${idx}`} style={styles.garminSection}>
                                    <Text style={[styles.sectionTitle, {marginBottom: 4}]}>
                                        <SportIcon activityType={actType} activityName={actName} size={15}
                                                   color="#38bdf8"/>
                                        {' '} ANALIZA BIEGU
                                    </Text>
                                    ...
                                    <View style={styles.paramsTable}>
                                        <View style={styles.paramRow}>
                                            ...
                                            <Text
                                                style={[styles.paramValue, {color: '#38bdf8'}]}>{formatCalculatedPace(gTime, gDist)}
                                                <Text style={styles.paramUnit}>/km</Text></Text>
                                        </View>
                                        <View style={styles.paramRow}>
                                            <Text style={styles.paramLabel}>Średnia Kadencja</Text>
                                            <Text style={styles.paramValue}>{cadence} <Text
                                                style={styles.paramUnit}>spm</Text></Text>
                                        </View>
                                        <View style={[styles.paramRow, {borderBottomWidth: 0}]}>
                                            <Text style={styles.paramLabel}>Up / Down</Text>
                                            <Text style={styles.paramValue}>{elevationGain} / {elevationLoss} <Text
                                                style={styles.paramUnit}>m</Text></Text>
                                        </View>
                                    </View>
                                            ...
                                            {!lapsCollapsed && (
                                                <View style={styles.table}>
                                                    <View style={styles.tableHeaderRow}>
                                                        ...
                                                        <Text
                                                            style={[styles.tableHeaderText, styles.colDyst]}>DYSTANS</Text>
                                                        <Text
                                                            style={[styles.tableHeaderText, styles.colCzas]}>CZAS</Text>
                                                        <Text
                                                            style={[styles.tableHeaderText, styles.colPace]}>TEMPO</Text>
                                                        <Text
                                                            style={[styles.tableHeaderText, styles.colHr]}>ŚR.HR</Text>
                                                        <Text
                                                            style={[styles.tableHeaderText, styles.colMaxHr]}>MAX.HR</Text>
                                                        <Text
                                                            style={[styles.tableHeaderText, styles.colKad]}>KAD.</Text>
                                                        <Text
                                                            style={[styles.tableHeaderText, styles.colKrok]}>KROK</Text>
                                                    </View>

                                                    {laps.map((lap, lIdx) => (
                                                        <View key={lIdx}
                                                              style={[styles.tableRow, lIdx % 2 !== 0 ? styles.rowOdd : styles.rowEven]}>
                                                            <Text
                                                                style={[styles.tableCell, styles.colL, {color: '#64748b'}]}>{lap.numer}</Text>
                                                            <Text
                                                                style={[styles.tableCell, styles.colDyst]}>{formatDistanceKm(lap.dystans)} km</Text>
                                                            <Text
                                                                style={[styles.tableCell, styles.colCzas]}>{formatTimeSeconds(lap.czas)}</Text>
                                                            <Text
                                                                style={[styles.tableCell, styles.colPace, {color: '#38bdf8'}]}>{formatDirectPaceSeconds(lap.tempo)}</Text>
                                                            <Text
                                                                style={[styles.tableCell, styles.colHr]}>{lap.tetno || '--'}</Text>
                                                            <Text
                                                                style={[styles.tableCell, styles.colMaxHr]}>{lap.tetno_max || '--'}</Text>
                                                            <Text
                                                                style={[styles.tableCell, styles.colKad]}>{lap.kadencja || '--'}</Text>
                                                            <Text
                                                                style={[styles.tableCell, styles.colKrok]}>{lap.krok ? parseFloat(lap.krok).toFixed(2) : '--'}</Text>
                                                        </View>
                                                    ))}
                                                </View>
                                            )}
                        ...
                        {showFeedback && (
                            <View style={styles.section}>
                                <Text style={styles.sectionTitle}><Ionicons name="chatbubble-ellipses" size={14}/> TWÓJ
                                    FEEDBACK</Text>
                                ...
                                <View style={styles.chipsContainer}>
                                    {FATIGUE_OPTIONS.map(opt => (
                                        ...
                                        >
                                            <Text
                                                style={[styles.chipText, zmeczenie === opt.value && styles.chipTextActive]}>{opt.label}</Text>
                                        </TouchableOpacity>
                                    ))}
                                </View>
                                ...
                                <TouchableOpacity style={[styles.saveBtn, loading && styles.disabled]}
                                                  onPress={submitFeedback} disabled={loading}>
